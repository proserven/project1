...
@Aspect // This tells Spring that this is an aspect class   // This is a class that will be used to advise other classes
@Component // This tells Spring to create an instance of this class and inject it into other classes
public class AuthAspect {

    @Autowired // This will autowire a proxy object, since the HttpServletRequest uses
    // one of the Web-Aware bean scopes // Specifically, the Request Scope
    ...
    private HttpServletRequest req; // This is a ***PoinCut*** There are several kinds of advice: Before, After, and Around
    // After also has 2 subtypes: AfterReturning & AfterThrowing        // Each of these has an annotation
    ...
    public Object authenticate(ProceedingJoinPoint pjp, Authorized authorized) throws Throwable {
        ...
        User currentUser = (User) session.getAttribute("currentUser"); // This cast is potentially dangerous
        // Make sure that anything you put in the currentUser attribute is of type User
        Role currentRole = currentUser.getRole();

        List<Role> allowedRoles = Arrays.asList(authorized.allowedRoles()); // This is a ***PointCut***
